...
class ReportService {
    static generateLotReport(runId) {
        ...
        const devices = deviceIds.map(id => store_1.db.devices.get(`${runId}:${id}`)).filter(Boolean);
        const runAlerts = Array.from(store_1.db.alerts.values()).filter(a => a.runId === runId);
        const auditChain = audit_service_1.AuditService.verifyRunChain(runId);
        ...
        const deviceDispositions = devices.map(d => {
            const dispo = store_1.db.dispositions.get(d.id);
            if (dispo) {
                if (dispo.decision === 'accept')
                    accepted++;
                else if (dispo.decision === 'reject')
                    rejected++;
                else if (dispo.decision === 'hold_fa')
                    holdFA++;
            }
            else {
                pending++;
